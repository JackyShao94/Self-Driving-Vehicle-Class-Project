...
x_curr = x0;
T = [];
x_kept = x0; % x used for simulation
t_kept = 1; % idx of x
num_del = 5000;
%%
% try
for l = 1:1:(Num_sect-1)
    z0=zeros(Ndec,1);
    for i = 1:N+1
        z0(2*N+8*(i-1)+(2:7))=x_curr;
    end
...
%     theta2 = max(TestTrack.theta(l+1),TestTrack.theta(l+2));
    ...
    if(l<=Num_sect-5)
        ...
        theta = TestTrack.theta(l+[0,0,0,0,0,0])'; % the section before terminal
    end
...
%%%
    ...
    [X_opt,FVAL,EXITFLAG,OUTPUT]=fmincon(@(z) cost_center(z,Ndec,Pc_plus,theta),z0,A,b,Aeq,beq,lb,ub,@(z) mycon(z,N,delta_s),options);
%%%
    % extract t,u,x,y from optimization result
    [t,u,x,y,v_u,v_v] = ext_result(X_opt,N); 
...
%     [x_curr_plus,x_real,y_real] = simu_section(t_discrete,u_discrete,x_curr);
    ...
    axis equal;
    title(['vel: ',num2str(x_curr_plus(2)),' dT: ',num2str(t_discrete(end)),...
        ' T: ',num2str(time(end))])
    drawnow;
    %update 
%     U = [U;u_discrete];

    time = time + t_discrete(end);
    x_curr = x_curr_plus;
    
    x_kept = [x_kept, x_curr];
    t_kept = [t_kept;length(T)];
%     pause;
end
% 
% catch
